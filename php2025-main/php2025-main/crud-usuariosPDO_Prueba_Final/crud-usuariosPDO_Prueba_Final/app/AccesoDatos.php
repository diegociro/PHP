...
class AccesoDatos
{
    ...
    public static function closeModelo()
    {
        if (self::$modelo != null) {
            ...
            $obj->stmt_updateSaldo = null;
            $obj->stmt_updateBloqueo = null;
            $obj->dbh = null;
    ...
    {
        $tuser = [];
        $this->stmt_usuarios->setFetchMode(PDO::FETCH_CLASS, 'Usuario'); //Convierte cada fila en un objeto Usuario
        ...
        return $tuser; // devuelve un array con todos los usuarios
    }
    ...
    // UPDATE
    public function modUsuario($user): bool //Actualiza nombre, password y comentario del usuario cuyo login coincida
    {
    ...
    //INSERT
    public function addUsuario($user): bool //Inserta un nuevo usuario en la base de datos
    {
    ...
    //tuser es una lista de logins que se saca del formulario (funciones.php)
    //Para cada login: Aumenta el saldo un 10%.
    {
    ...
    //$bloqueoUsuario es una lista de logins que deben quedar bloqueados
    public function actualizarBloqueos(array $bloqueoUsuario)
    {
        //Obtiene todos los usuarios de la base de datos, con todos sus datos porque
        //necesita recorrer todos los usuarios, no solo los seleccionados
        ...
        foreach ($tuser as $user) {
            ...
            if (in_array($user->login, $bloqueoUsuario)) {
                ...
                // No está la lista y estaba bloqueado, lo desbloqueo
    ...
    {
        // Ejecuto la consulta directamente, todos desbloqueados
        $this->dbh->exec("UPDATE Usuarios SET bloqueo = 0");
        // Ojo con las comillas simples
        $listalogin = implode("','", $bloqueoUsuarios); //construyo una lista separada por comas y los elementos entre '' 
        // ¡OJO! Esta orden no es inmune a la inyección de SQL
        $this->dbh->exec(
            "UPDATE Usuarios SET bloqueo = 1 WHERE login IN ('$listalogin')"
        );
    }
